...
However, we note that centering X and Y at the start of the procedure and not fitting an intercept in each SER is equivalent to not centering X and Y and fitting an intercept in each SER step. As a result, in the logistic case, we do fit an intercept in each logistic SER. In order to obtain a final intercept value (e.g. for predictive purposes), we take our final estimate for $\hat{b_j} = \sum_{l=1}^L \hat{\mu_{lj}} \alpha_{lj}$ (call is `b_hat`), and fit one more logistic regression:
```
...
Here, we see some instances where the SuSiE models disagree quite a bit in the middle of the curve (top and bottom rows) when the regular linear and logistic models still seem to agree.
