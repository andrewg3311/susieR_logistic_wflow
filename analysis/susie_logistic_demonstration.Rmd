...
However, we note that centering X and Y at the start of the procedure and not fitting an intercept in each SER is equivalent to not centering X and Y and fitting an intercept in each SER step. As a result, in the logistic case, we do fit an intercept in each logistic SER. In order to obtain a final intercept value (e.g. for predictive purposes), we take our final estimate for $\hat{b_j} = \sum_{l=1}^L \hat{\mu_{lj}} \cdot \alpha_{lj}$ (call is `b_hat`), and fit one more logistic regression:
```
glm(Y ~ 1 + offset(X %*% b_hat), family = "binomial")
```
...
For example, consider the easy case where $n = 1000$, $p = 10$, each entry $X_{ij} \stackrel{iid}{\sim} \mathcal{N}(0, 1)$, and the first three effects are iid $\mathcal{N}(0, 1)$. I test when I set $L = 3$ (the optimal value), $L = 5$, and $L = 10$.
