...
update_b_l = function(X, Y, xi, prior_weights, V, Sigma2, Mu, Alpha, l) {
  ...
  for (j in 1:ncol(X)) {
    ...
    # update Alpha[j, l]
    Alpha[j, l] = log(prior_weights[j]) + ((num^2) / (common_denom^2)) + (1/2)*log(1 / common_denom) # on log-scale, for stability
...
$$
\alpha_j \propto \pi_j \frac{\mathcal{N}(\hat{b_j}; 0, \sigma_0^2 + \sigma_{MLE_j}^2)}{\mathcal{N}(\hat{b_j}; 0, \sigma_0^2 )} = \pi_j \sqrt{\frac{\sigma_0^2}{\sigma_0^2 + \sigma_{MLE_j}^2}} \exp\Bigg\{\frac{\hat{b_j}^2}{2}\Big(\frac{1}{\sigma_0^2} - \frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}\Big)\Bigg\} \propto \pi_j \sqrt{\frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}} \exp\Bigg\{\frac{\hat{b_j}^2}{2}\Big(\frac{1}{\sigma_0^2} - \frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}\Big)\Bigg\}
$$

In the VB algorithm, we have
$$
\alpha_j \propto \pi_j \sqrt{\sigma_{1j}^2} \exp\Bigg\{\frac{\mu_{1j}^2}{\sigma_{1j}^4}\Bigg\}
$$

Now, if we start from the PIPs from the GLM version and plug in our results when applied to the VB algorithm:
$$
\begin{aligned}
\alpha_j \propto \pi_j \sqrt{\frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}} \exp\Bigg\{\frac{\hat{c_j}^2}{2}\Big(\frac{1}{\sigma_0^2} - \frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}\Big)\Bigg\} = \pi_j \sqrt{\frac{1}{\sigma_0^2 + \frac{1}{\sum_{i=1}^n\frac{1}{\xi_i}(g(\xi_i) - \frac12)x_{ij}^2}}} \exp\Bigg\{\frac{1}{2} \Bigg[\frac{\sum_{i=1}^n x_{ij}\Big[y_i - \frac12 - \frac{1}{\xi_i}(g(\xi_i) - \frac12)x_i^T\bar{b_{-l}}\Big]}{\sum_{i=1}^n x_{ij}^2 \frac{1}{\xi_i}(g(\xi_i) - \frac12)}\Bigg]^2\Bigg(\frac{1}{\sigma_0^2} - \frac{1}{\sigma_0^2 + \frac{1}{\sum_{i=1}^n\frac{1}{\xi_i}(g(\xi_i) - \frac12)x_{ij}^2}}\Bigg)\Bigg\} = \\
