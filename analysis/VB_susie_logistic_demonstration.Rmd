...
$$
\begin{aligned}
q(\gamma_l = j) \propto \pi_j \cdot \exp \Bigg\{\frac{\nu_{jl}^2}{2\tau_{jl}}\Bigg\} \\
...
update_b_l = function(X, Y, xi, prior_weights, V, Sigma2, Mu, Alpha, l) {
  ...
  for (j in 1:ncol(X)) {
    ...
    Alpha[j, l] = log(prior_weights[j]) + (num^2 / denom) # on log-scale, for stability
...
If we interpret $\nu_{jl} \iff \frac{\hat{b_j}}{\sigma_{MLE_j}^2}$, then the SER update can be represented as:
$$
\begin{aligned}
\mu_{1j} = \nu_{jl} \cdot \sigma_{1j}^2 \\
\sigma_{1j}^2 = ??
...
$$
\alpha_j \propto \pi_j \frac{\mathcal{N}(\hat{b_j}; 0, \sigma_0^2 + \sigma_{MLE_j}^2)}{\mathcal{N}(\hat{b_j}; 0, \sigma_0^2 )} = \pi_j \sqrt{\frac{\sigma_0^2}{\sigma_0^2 + \sigma_{MLE_j}^2}} \exp\Bigg\{\frac{\hat{b_j}^2}{2}\Big(\frac{1}{\sigma_0^2} - \frac{1}{\sigma_0^2 + \sigma_{MLE_j}^2}\Big)\Bigg\}
$$

In the VB algorithm, we have
$$
\alpha_j \propto \pi_j \exp\Bigg\{\frac{\nu_{jl}^2 \sigma_{1j}^2}{2}\Bigg\} = \pi_j exp\Bigg\{\frac{\mu_{1j}^2}{2\sigma_{1j}^2}\Bigg\} \approx \pi_j exp\Bigg\{\frac{\hat{b_j}^2 \sigma_{1j}^2}{2\sigma_{MLE_j}^4}\Bigg\}
$$
